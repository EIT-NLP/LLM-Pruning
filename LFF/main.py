...
def parse_args():
    ...
    # 添加命令行参数
    parser.add_argument("--regu_weight", type=float, default=1.0) # 低秩网络的秩
    parser.add_argument("--lw_net_rank", type=float, default=0.65, help="the rank of the lightweght network") # 低秩网络的秩
    parser.add_argument("--init_router", action="store_true") # 是否训练低秩网络
    parser.add_argument("--retrain_lw", action="store_true") # 是否训练低秩网络
    parser.add_argument("--retrain_router", action="store_true") # 是否训练低秩网络
    parser.add_argument("--train_lw_only", action="store_true") # 是否训练低秩网络
    parser.add_argument("--use_retrain_lff", action="store_true") # 是否训练低秩网络
    parser.add_argument("--train_router_only", action="store_true") # 是否训练路由器
    parser.add_argument("--train_lw_and_router", action="store_true") # 是否训练路由器
    parser.add_argument("--post_norm_router", action="store_true") 
    parser.add_argument("--skipgpt", action="store_true") 
    parser.add_argument("--lora_hc", action="store_true")
    parser.add_argument("--eval_stage2", action="store_true")
    ...
    parser.add_argument("--model_name", type=str, default="Llama-3.1-8b", help="Name of the model to load") # 模型名称
    parser.add_argument("--model_path", type=str, default="/data/chao_han/model/Meta-Llama-3_1-8B", help="Path to the model to load") # 模型路径
    # parser.add_argument("--model_path", type=str, default="/data/chao_han/model/Llama-3.2-3b", help="Path to the model to load") # 模型路径
    parser.add_argument("--finetuned", action="store_true", help="Load a finetuned model") # 是否加载微调后的模型
    parser.add_argument("--method", type=str, default="router_attn_mlp", choices=['router_attn_mlp', 'mod_twice', 'mod', "router_all","post_training","original","joint","post_training_deepspeed", "train_lw_only"], help="choosed method") # 动态层跳过的方法
    parser.add_argument("--dataset_path", type=str, default="./RedPajama-Data-1T-Sample", help="Path to the dataset") # 数据集路径
    ...
    parser.add_argument("--max_steps_stage", type=int, default=2000, help="Maximum training steps") # 最大训练步数
    ...
    parser.add_argument("--evaluation_strategy", type=str, default="no", help="Evaluation strategy") # 评估策略
    parser.add_argument("--eval_steps", type=int, default=100, help="Evaluation steps") # 评估步数
    ...
    parser.add_argument("--max_length", type=int, default=2048, help="the max context length") # 最大上下文长度
