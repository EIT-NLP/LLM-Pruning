...
class CustomTrainer_lw(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        loss = super().compute_loss(model, inputs, return_outputs=return_outputs)
        lw_attn_loss = 0.0
        lw_mlp_loss = 0.0
        for layer in model.model.layers:
            if isinstance(layer, router_attn_mlp_llama) or isinstance(layer, router_attn_mlp_llama_dllm):
                lw_attn_loss_layer, lw_mlp_loss_layer = layer.return_lw_loss()
                if lw_attn_loss_layer is not None and lw_mlp_loss_layer is not None:
                    lw_attn_loss += lw_attn_loss_layer
                    lw_mlp_loss += lw_mlp_loss_layer  
                layer.reset_lw_loss() 
        return lw_attn_loss + lw_mlp_loss
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.model.training:
            ...
            if "Llama" in model.__class__.__name__:
                ...
                for layer in model.model.layers:
                    if isinstance(layer, router_attn_mlp_llama) or isinstance(layer, router_attn_mlp_llama_dllm):
                        ...
                        mlp_router_loss += layer.mlp_router_zero_prob
                        num_layers += 1  # 记录层数
                        regu_loss += layer.regu_loss
                    # else: # 对于原始层将稀疏度记为0
                    #     attn_router_loss += 1.0
                    #     mlp_router_loss += 1.0
                    #     num_layers += 1 
            ...
            if num_layers > 0:
                ...
                attn_router_loss /= num_layers
                mlp_router_loss /= num_layers
                regu_loss /= num_layers


            
            total_router_loss = (attn_router_loss + mlp_router_loss) / 2
            # total_router_loss = mlp_router_loss
            ...
            if self.custom_args.train_method=="curriculum":
                ...
                # 原loss
                loss = loss + 8*abs((1-self.custom_args.sparsity)-total_router_loss) + self.custom_args.regu_weight * regu_loss # 可以调节正则化项的权重

                # 分别约束attn稀疏度和ffn稀疏度
                # loss = loss + 8*abs((1-self.custom_args.sparsity)-attn_router_loss) + 8*abs((1-self.custom_args.sparsity)-mlp_router_loss)

                # 启发性loss,分别约束attn和mlp的稀疏率
                # loss = loss + 20*0.5*(abs((1-self.custom_args.sparsity)-attn_router_loss) + abs((1-self.custom_args.sparsity)-mlp_router_loss)) # 可以调节正则化项的权重   

                # if total_router_loss > (1 - self.custom_args.sparsity):
                #     loss = loss + 5*(total_router_loss-(1-self.custom_args.sparsity)) # 如果超过目标稀疏率，才增加惩罚
                # else:
                #     loss = loss
        return loss
