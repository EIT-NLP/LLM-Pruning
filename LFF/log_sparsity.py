...
from dllm import router_attn_mlp_llama_dllm
...
class SparsityLoggingCallback_lw(TrainerCallback):
    def __init__(self, log_dir='logs'):
        self.writer = SummaryWriter(log_dir=log_dir)
    def on_step_end(self, args, state, control, **kwargs):
        model = kwargs.get('model', None)
        if model is not None:
            total_attn_sparsity = 0
            total_mlp_sparsity = 0
            total_attn_lw_sparsity = 0
            total_mlp_lw_sparsity = 0
            total_mlp_cos_ori = 0
            total_mlp_cos_lw = 0
            total_attn_cos_ori = 0
            total_attn_cos_lw = 0
            num_layers = 0
            attn_sparsity_layer = []
            mlp_sparsity_layer = []
            if "Llama" in model.__class__.__name__ or 'Peft' in model.__class__.__name__:
                # 遍历模型中的所有层并累积稀疏度
                if "Llama" in model.__class__.__name__ :
                ...
                for layer in layers:  
                    if isinstance(layer, router_attn_mlp_llama) or isinstance(layer, router_attn_mlp_llama_dllm):
                        attn_sparsity, mlp_sparsity, attn_lw_sparsity, mlp_lw_sparsity= layer.compute_sparsity()
                        total_attn_sparsity += attn_sparsity
                        total_mlp_sparsity += mlp_sparsity
                        total_attn_lw_sparsity += attn_lw_sparsity
                        total_mlp_lw_sparsity += mlp_lw_sparsity
                        
                        attn_sparsity_layer.append(attn_sparsity)
                        mlp_sparsity_layer.append(mlp_sparsity)
                        
                        mlp_cos_ori, mlp_cos_lw, attn_cos_ori, attn_cos_lw = layer.return_cos_mlp()
                        if mlp_cos_ori is not None and mlp_cos_lw is not None:
                            total_mlp_cos_ori += mlp_cos_ori
                            total_mlp_cos_lw += mlp_cos_lw
                            total_attn_cos_ori += attn_cos_ori
                            total_attn_cos_lw += attn_cos_lw
                        num_layers += 1

                # 计算平均稀疏度
                average_attn_sparsity = total_attn_sparsity / num_layers if num_layers > 0 else 0
                average_mlp_sparsity = total_mlp_sparsity / num_layers if num_layers > 0 else 0
                average_attn_lw_sparsity = total_attn_lw_sparsity / num_layers if num_layers > 0 else 0
                average_mlp_lw_sparsity = total_mlp_lw_sparsity / num_layers if num_layers > 0 else 0

                # 记录到 wandb
                # wandb.log({
                #     "Average Attention Sparsity": average_attn_sparsity,
                #     "Average MLP Sparsity": average_mlp_sparsity,
                #     "Average Attention LW Sparsity": average_attn_lw_sparsity,
                #     "Average MLP LW Sparsity": average_mlp_lw_sparsity,
                #     "epoch": state.epoch
                # })
                ...
                self.writer.add_scalar("Average_Attention_Sparsity", average_attn_sparsity, state.global_step)
                self.writer.add_scalar("Average_MLP_Sparsity", average_mlp_sparsity, state.global_step)
                self.writer.add_scalar("Average_Attention_LW_Sparsity", average_attn_lw_sparsity, state.global_step)
                self.writer.add_scalar("Average_MLP_LW_Sparsity", average_mlp_lw_sparsity, state.global_step)
                self.writer.add_scalar("epoch", state.epoch, state.global_step)

                if state.global_step % 10 == 0:
                    # print(1)
                    print(f"Average_Attention_Sparsity: {average_attn_sparsity}, Average_MLP_Sparsity: {average_mlp_sparsity}, ",
                          f"Average_Attention_LW_Sparsity: {average_attn_lw_sparsity}, Average_MLP_LW_Sparsity: {average_mlp_lw_sparsity}",
                          f"Average_MLP_Cos_ori: {total_mlp_cos_ori/num_layers}, Average_MLP_Cos_lw: {total_mlp_cos_lw/num_layers}"
                          f"Average_Attention_Cos_ori: {total_attn_cos_ori/num_layers}, Average_Attention_Cos_lw: {total_attn_cos_lw/num_layers}") 
                    print(f'layerwise attn sparsity: {attn_sparsity_layer}')
                    print(f'layerwise mlp sparsity: {mlp_sparsity_layer}')
                # 重置所有层的稀疏度计数
                for layer in layers:
                    if isinstance(layer, router_attn_mlp_llama):
                        layer.reset_sparsity_counts()
