...
class LowRankMLPKernel(_PruningMLPKernel):
    ...
    def base_prefill(
        ...
        w_gate_r1: Optional[torch.Tensor]=None,
        w_gate_r2: Optional[torch.Tensor]=None,
        w_down_r1: Optional[torch.Tensor]=None,
        w_down_r2: Optional[torch.Tensor]=None,
        activation: Optional[str]='identity',
        **kwargs,
    ):
        if w_gate_r1 is None: # single mlp layer
            low_rank = F.linear(x, w_up_r1, kwargs.get('b_up_r1', None)) # [M, r]
            low_rank = F.linear(low_rank, w_up_r2, kwargs.get('b_up_r2', None)) # [M, N]
        
        else:
            assert w_gate_r1 is not None and w_down_r1 is not None # full FFN
            low_rank_up = F.linear(x, w_up_r1, kwargs.get('b_up_r1', None)) # [M, r]
            low_rank_up = F.linear(low_rank_up, w_up_r2, kwargs.get('b_up_r2', None)) # [M, N1]

            low_rank_gate = F.linear(x, w_gate_r1, kwargs.get('b_gate_r1', None)) # [M, r]
            low_rank_gate = F.linear(low_rank_gate, w_gate_r2, kwargs.get('b_gate_r2', None)) # [M, N1]

            low_rank = ACT2FUNC.get(activation, lambda x: x)(low_rank_gate) * low_rank_up
            low_rank = F.linear(low_rank, w_down_r1, kwargs.get('b_down_r1', None)) # [M, r]
            low_rank = F.linear(low_rank, w_down_r2, kwargs.get('b_down_r2', None)) # [M, K]

            if w_up.numel() > 0: # retained neuron
                retained_up = F.linear(x, w_up, b_up) # [M, N0]
                retained_gate = F.linear(x, w_gate, b_gate) # [M, N0]
                retained = ACT2FUNC.get(activation, lambda x: x)(retained_gate) * retained_up
                retained = F.linear(retained, w_down, b_down) # [M, K]
