...
#       Cache
###################
class DynamicLayerSeqFirstLayer(CacheLayerMixin):
    """
...
class DynamicCacheSeqFirst(Cache):
    ...
    def __init__(
        ...
        layers = [DynamicLayerSeqFirstLayer() for _ in range(config.num_hidden_layers)]

        # If neither of config nor ddp_data was passed, then simply lazy init a full cache of DynamicLayer
        if len(layers) == 0:
            super().__init__(
                layer_class_to_replicate=DynamicLayerSeqFirstLayer,
            )
        else:
