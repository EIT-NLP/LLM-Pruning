    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.router_ffn is not None:
            ...
            selected_mask_ffn = skip_ffn * attention_mask
...
class WrapperModel(BaseWrapperModel):
    ...
    def compute_sparsity(self) -> dict[str, torch.Tensor]:
        ...
        if 'ffn' in self.skipped_tokens:
            sparsity['ffn'] = (self.skipped_tokens['ffn'] / self.config.num_hidden_layers) / self.total_tokens if self.total_tokens > 0 else 0
        
        if 'attention' in sparsity and 'ffn' in sparsity:
            sparsity['total'] = (sparsity['attention'] + sparsity['ffn']) / 2
