...
struct SharedMemoryQ {
    ...
    alignas(16) uint32_t cta_reduce_cache[kWarps + 1];
};
...
>
__global__ void attention_head_v0_kernel(const __grid_constant__ AttentionParams<TMADescK, TMADescV> params) {
    using namespace cute;
    ...
    if constexpr (IsCausal) {
        ...
        if (lane_id == 0) smem_q.cta_reduce_cache[warp_id] = warp_max_query;
        __syncthreads();

        uint32_t x = 0;
        if (warp_id == 0 && lane_id < kWarps) x = smem_q.cta_reduce_cache[lane_id];
        if (warp_id == 0) {
            x = __reduce_max_sync(0xffffffff, x);
            if (lane_id == 0) smem_q.cta_reduce_cache[kWarps] = x;
        }
        __syncthreads();
        const uint32_t cur_max_query = smem_q.cta_reduce_cache[kWarps];
    ...
    for (int32_t k_tile=0; k_tile < key_tile_num; ++k_tile) {
        ...
        for (uint32_t mi=0; mi < size<1>(rP); ++mi) {
            ...
            CUTE_UNROLL
                ...
                uint32_t abs_k_pos = (key_start_tile + k_tile) * BN + n;
                ...
                if constexpr (IsCausal) {
                    ...
                    rP(mma_i * 2, mi, ni) = valid1 ? rP(mma_i * 2, mi, ni) * scale_ln2 : -MAXFLOAT;
                    rP(mma_i * 2 + 1, mi, ni) = valid2 ? rP(mma_i * 2 + 1, mi, ni) * scale_ln2 : -MAXFLOAT;
        ...
        //     auto pv_tile_mnk = tile_shape(tiled_mma_pv{});
        //     auto pv_a_mk = select<0, 2>(pv_tile_mnk);
        //     Tensor pv_a_ref = make_identity_tensor(pv_a_mk);
        //     Tensor pv_a_tv = composition(pv_a_ref, tiled_mma_pv{}.get_layoutA_TV());

        //     print("% PV-A full TV layout\n");
        //     print_latex_tv(pv_a_tv, pv_a_mk);
        //     print("% rPlA k-block 0 fragment storage layout\n");
        //     print_latex(rPlA(_, _, _0{}).layout());
        //     print("% cPlA k-block 0 coordinate tensor descriptor: ");
        //     print(cPlA(_, _, _0{})); print("\n");
        // }
    ...
    for (uint32_t mi=0; mi < size<1>(rO); ++mi) {
        ...
        for (uint32_t mma_i=0; mma_i < size<0>(rO) / 2; ++mma_i) {
            CUTE_UNROLL
            for (uint32_t ni=0; ni < size<2>(rO); ++ni) {
                rO(mma_i * 2, mi, ni) /= rL(mi * 2 + mma_i);
                rO(mma_i * 2 + 1, mi, ni) /= rL(mi * 2 + mma_i);
            }
        }
    }
...
struct Attention_Head_V0_Host {
    static void run(
        ...
        constexpr auto kernel = attention_head_v0_kernel<
