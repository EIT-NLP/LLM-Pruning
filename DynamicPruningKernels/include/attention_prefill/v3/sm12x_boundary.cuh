...
>
__global__ __launch_bounds__(256) void attention_head_v3_boundary(const __grid_constant__ AttentionParams<TMADescK, TMADescV> params) {
            ...
            for (uint32_t mi=0; mi < size<1>(rP); ++mi) {
                ...
                for (uint32_t mma_i=0; mma_i < size<0>(rP) / 2; ++mma_i) {
                    ...
                    for (uint32_t ni=0; ni < size<2>(rP); ++ni) {
                        ...
                        const bool valid1 = (abs_k_pos >= key_start) && (abs_k_pos < key_end);
                        const bool valid2 = (abs_k_pos + 1 >= key_start) && (abs_k_pos + 1 < key_end);
