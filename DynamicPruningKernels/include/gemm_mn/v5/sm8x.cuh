...
struct WarpLayoutTraits {
    ...
    static constexpr uint32_t G2SBIter = cute::ceil_div(kBN, G2SColPerCTA);
...
>
__global__ void gemm_mn_v5_kernel(const __grid_constant__ GEMMParams params) {
    ...
    //
    SkipHelper<G2SAIter, MMAIter> skip_helper;
    CUTE_UNROLL
    for (uint32_t i=0, off=thread_off_m; i < G2SAIter; ++i, off+=G2SRowPerCTA) {
        skip_helper.rMask[i] = off < M ? mMask(make_coord(off, bidy / NGIter)) : 0;
        skip_helper.rIndex_ld[i] = off < M ? static_cast<uint32_t>(mIndex(make_coord(off, bidy / NGIter))) : 0;
    }
    ...
    for (uint32_t i=0; i < G2SAIter; ++i) {
        ...
        // skip_helper.execute_g2s_warp[i] = static_cast<uint8_t>(__any_sync(0xffffffff, static_cast<uint8_t>(skip_helper.rMask[i] > 0)));
    }
    ...
    for (
        ...
        skip_helper.rIndex_st[i*2] = (off < M && mMask(make_coord(off, bidy / NGIter)) > 0) ? static_cast<uint32_t>(mIndex(make_coord(off, bidy / NGIter))) : M;
        skip_helper.rIndex_st[i*2+1] = (off + 8 < M && mMask(make_coord(off + 8, bidy / NGIter)) > 0) ? static_cast<uint32_t>(mIndex(make_coord(off + 8, bidy / NGIter))) : M;
    ...
    //     cute::print(gA); cute::print("\n");
    //     cute::print(gB); cute::print("\n");
    // }

    // fill the pipeline
    uint32_t producer_stage = producer % PipelineSmem;
    CUTE_UNROLL
    for (uint32_t p=0; p < PipelineSmem - 1; ++p) {
        ...
        for (uint32_t i=0; i < size<1>(pDsA); ++i) {
            ...
            // if (tidx == 0 && (bidx + bidy + bidz == 0) && i + p == 0) {
            //     cute::print(pSgA); cute::print("\n");
            //     cute::print(pDsA); cute::print("\n");
        ...
        cp_async_fence();
        ++producer;
        producer_stage = (producer_stage + 1) % PipelineSmem;
    }
    ...
    // if (tidx == 0 && (bidx + bidy + bidz == 0)) {
    //     cute::print(pSsA); cute::print("\n");
    //     cute::print(pDrA); cute::print("\n");
    //     cute::print(pSsB); cute::print("\n");
    //     cute::print(pDrB); cute::print("\n");
    // }

    const uint32_t k_tile_num = SplitKSize / BK;
    uint32_t consumer_stage = consumer % PipelineSmem;
    // ready for smem -> rmem pipeline
    cp_async_wait<kPipelineSmem - 2>();
    __syncthreads();
    ...
    for (uint32_t k_tile=0; k_tile < k_tile_num; ++k_tile) {
        ...
        for (uint32_t ldsm=0; ldsm < reg_tile_num; ++ldsm) {
            ...
            if (ldsm == reg_tile_num - 1) {
                ...
                __syncthreads();
                consumer_stage = (consumer_stage + 1) % PipelineSmem;
            }
    ...
    if (SplitK == 1) {
        ...
        for (uint32_t i=0; i < size<1>(pSsD); ++i) {
            if (skip_helper.rMask[i]) {
                ...
                //     cute::print(pSsD); cute::print("\n");
                //     cute::print(pDgD); cute::print("\n");
                //     cute::print(gD_epi); cute::print("\n");
                // }
    ...
    // split-k atomic-add write back
        ...
        for (uint32_t i=0; i < size<1>(rD); ++i) {
            ...
            for (
                ...
                // if (tidx == 0 && bidx + bidy + bidz == 0 && i == 0 && j == 0) {
                //     cute::print(gD_col_off); cute::print("\n");
                //     cute::print(rD); cute::print("\n");
                //     cute::print(mD); cute::print("\n");
                // }
            }
        }
    }
...
struct GEMM_MN_V5_Host {
    static void run(
        ...
        constexpr auto kernel = gemm_mn_v5_kernel<
