...
__device__ __forceinline__ void update_skip_metadata(
    ...
    if constexpr (kWarpSkipMMA) {
        ...
        for (
            ...
            uint32_t rIndex_st1 = (off < M && mMask(make_coord(off, group_id)) > 0) ? static_cast<uint32_t>(mIndex(make_coord(off, group_id))) : M;
            uint32_t rIndex_st2 = (off + 8 < M && mMask(make_coord(off, group_id)) > 0) ? static_cast<uint32_t>(mIndex(make_coord(off + 8, group_id))) : M;
...
__global__ void gemm_mn_v1_kernel(const __grid_constant__ GEMMParams params) {
    ...
    update_skip_metadata<G2SAIter, kWarpSkipG2S, kWarpSkipMMA, G2SRowPerCTA, MMARow, MMACol, MMAIter, PipelineSmem, decltype(mMask), decltype(mIndex)>(
        skip_helper, mMask, mIndex, M, max_off_m, thread_off_m, base_off_m, group_id, warp_id, lane_id
    );
