...
>
__global__ __launch_bounds__(256) void attention_head_v0_boundary(const __grid_constant__ AttentionParams<TMADescK, TMADescV> params) {
    ...
    using sPLayout = typename MMATraits_::sPLayout;
    ...
    using r2sP_tiled_copy = typename MMATraits_::r2sP_tiled_copy;
    using s2rP_tiled_copy = typename MMATraits_::s2rP_tiled_copy;
    ...
    using SharedStorageKV = SharedMemoryKV<DType, sKLayout, sVLayout, sPLayout, PipelineK, PipelineV, IsCTAReduce, MMACol>;
    ...
    // Split only the tiles that overlap the valid KV interval.  This skips
    // fully left-padded tiles while keeping split boundaries tile-aligned.
    const uint32_t valid_key_start = IsLeftpad ? cute::min(mLeftpad(bidz), Tk) : 0;
    const uint32_t sequence_key_start_tile = valid_key_start / BN;
    const uint32_t sequence_key_end_tile = cute::ceil_div(Tk, BN);
    const uint32_t total_k_tiles = valid_key_start < Tk
        ? sequence_key_end_tile - sequence_key_start_tile
        : 0;
    const uint32_t tiles_per_split = total_k_tiles / Split;
    const uint32_t extra_tiles = total_k_tiles % Split;
    const uint32_t key_tile_num = tiles_per_split + (bidx < extra_tiles);
    const uint32_t key_start_tile = sequence_key_start_tile
        + bidx * tiles_per_split
        + cute::min(bidx, extra_tiles);
    const uint32_t key_end_tile = key_start_tile + key_tile_num;
    const uint32_t key_start = cute::max(valid_key_start, key_start_tile * BN);
    const uint32_t key_end = cute::min(Tk, key_end_tile * BN);
    ...
    for (uint32_t k_tile=0; k_tile < key_tile_num; ++k_tile) {
        ...
        const bool boundary_check = tile_start < key_start || tile_end > key_end;
        ...
        // TMA zero-fills OOB data, but OOB scores must be excluded from the
        // softmax denominator.  Only the first/last boundary tile pays this.
        if (boundary_check) {
            CUTE_UNROLL
            for (uint32_t mi=0; mi < size<1>(rP); ++mi) {
                ...
                for (uint32_t mma_i=0; mma_i < size<0>(rP) / 2; ++mma_i) {
                    ...
                    for (uint32_t ni=0; ni < size<2>(rP); ++ni) {
                        ...
                        const uint32_t n = get<1>(mn);
                        const uint32_t abs_k_pos = tile_start + n;
                        const bool valid0 = abs_k_pos >= key_start && abs_k_pos < key_end;
                        const bool valid1 = abs_k_pos + 1 >= key_start && abs_k_pos + 1 < key_end;
                        rP(mma_i * 2, mi, ni) = valid0 ? rP(mma_i * 2, mi, ni) : -MAXFLOAT;
                        rP(mma_i * 2 + 1, mi, ni) = valid1 ? rP(mma_i * 2 + 1, mi, ni) : -MAXFLOAT;
                    }
                }
            }
        }
        ...
        // use warp reduction / CTA reduction based on warp num
        CUTE_UNROLL
        for (uint32_t mi=0; mi < size<1>(rP); ++mi) {
            CUTE_UNROLL
            for (uint32_t mma_i=0; mma_i < size<0>(rP) / 2; ++mma_i) {
        ...
        auto r2sP_thr_copy = r2sP_tiled_copy{}.get_slice(tidx);
        auto pSrP = r2sP_thr_copy.retile_S(rP);
        auto pDsP = r2sP_thr_copy.partition_D(sP);
        auto pSrP_cast = make_tensor_like<DType>(pSrP);
        copy(pSrP, pSrP_cast);
        copy(r2sP_tiled_copy{}, pSrP_cast, pDsP);
        ...
        auto s2rP_thr_copy = s2rP_tiled_copy{}.get_slice(tidx);
        auto pSsP = s2rP_thr_copy.partition_S(sP);
        auto pDrPlA = s2rP_thr_copy.retile_D(rPlA);
        copy(s2rP_tiled_copy{}, pSsP, pDrPlA);
    ...
    for (uint32_t mi=0; mi < size<1>(rO); ++mi) {
        ...
        for (uint32_t mma_i=0; mma_i < size<0>(rO) / 2; ++mma_i) {
            ...
            row_sum = issue_cta_reduce_sum(row_sum);
