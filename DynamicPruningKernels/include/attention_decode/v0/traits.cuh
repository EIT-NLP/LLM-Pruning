...
template <typename DType, class LayoutK, class LayoutV, class LayoutP, uint32_t PipelineK, uint32_t PipelineV, bool IsCTAReduce=false, uint32_t kReduceWarp=4>
struct SharedMemoryKV;

template <typename DType, class LayoutK, class LayoutV, class LayoutP, uint32_t PipelineK, uint32_t PipelineV, uint32_t kReduceWarp>
struct SharedMemoryKV<DType, LayoutK, LayoutV, LayoutP, PipelineK, PipelineV, false, kReduceWarp> {
    cute::ArrayEngine<DType, cute::cosize_v<LayoutK>> K;
    cute::ArrayEngine<DType, cute::cosize_v<LayoutV>> V;
    cute::ArrayEngine<DType, cute::cosize_v<LayoutP>> P;

    alignas(16) uint64_t tma_k_barrier[PipelineK];
    alignas(16) uint64_t tma_v_barrier[PipelineV];
};
template <typename DType, class LayoutK, class LayoutV, class LayoutP, uint32_t PipelineK, uint32_t PipelineV, uint32_t kReduceWarp>
struct SharedMemoryKV<DType, LayoutK, LayoutV, LayoutP, PipelineK, PipelineV, true, kReduceWarp> {
    cute::ArrayEngine<DType, cute::cosize_v<LayoutK>> K;
    cute::ArrayEngine<DType, cute::cosize_v<LayoutV>> V;
    cute::ArrayEngine<DType, cute::cosize_v<LayoutP>> P;
...
struct MMATraits<kBM, kBN, kBK, kPipelineK, kPipelineV, WarpLayoutTraits_, DType, kWarps, true> {
    ...
    using mma_pv_permutations = cute::Tile<cute::_16, cute::Int<BK>, cute::_16>;
    ...
    // The matrix path needs all 16 MMA rows, whereas the predicated path only
    // exchanges the BM live rows.
    using SwizzleLayoutAtomP = decltype(cute::composition(
        cute::Swizzle<3, 3, 3>{},
        cute::make_ordered_layout(
            cute::make_shape(cute::Int<MMA_M>{}, cute::Int<BN>{}),
            cute::make_step(cute::_1{}, cute::_0{})
        )
    ));
    using sPLayout = decltype(cute::tile_to_shape(
        SwizzleLayoutAtomP{},
        cute::make_shape(cute::Int<MMA_M>{}, cute::Int<BN>{}),
        cute::make_step(cute::_1{}, cute::_0{})
    ));
    ...
    // P register/shared-memory exchange between QK and PV warp ownership.
    using r2sP_atom = cute::Copy_Atom<cute::SM120_U32x4_STSM_N, DType>;
    using r2sP_tiled_copy = decltype(cute::make_tiled_copy_C(r2sP_atom{}, tiled_mma_qk{}));
    using s2rP_atom = cute::Copy_Atom<cute::SM75_U32x4_LDSM_N, DType>;
    using s2rP_tiled_copy = decltype(cute::make_tiled_copy_A(s2rP_atom{}, tiled_mma_pv{}));
