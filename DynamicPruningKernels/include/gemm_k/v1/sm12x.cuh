...
struct MmaTraits {
    ...
    using r2s_atom = cute::Copy_Atom<cute::SM120_U32x4_STSM_N, DType>;
...
__device__ __forceinline__ void update_skip_metadata(
    ...
    const uint8_t execute_cta = base_off_m < max_off_m
        ? static_cast<uint8_t>(mMask(make_coord(base_off_m, k_group_id)) != 0)
        : 0;
    ...
    const uint8_t full_cta = full_tile
        ? static_cast<uint8_t>(mMask(make_coord(max_off_m - 1, k_group_id)) != 0)
        : 0;
    if (full_cta) {
        const uint8_t group_all_active = static_cast<uint8_t>(mMask(make_coord(M - 1, k_group_id)) != 0);

        CUTE_UNROLL
        for (uint32_t i=0, off=thread_off_m; i < G2SAIter; ++i, off+=G2SRowPerCTA) {
            skip_helper.rMask[i] = 1;
            skip_helper.rIndex_ld[i] = group_all_active ? off : static_cast<uint32_t>(mIndex(make_coord(off, k_group_id)));
    ...
    CUTE_UNROLL
    for (uint32_t i=0, off=thread_off_m; i < G2SAIter; ++i, off+=G2SRowPerCTA) {
        const uint8_t mask = off < max_off_m ? mMask(make_coord(off, k_group_id)) : 0;
        skip_helper.rMask[i] = mask;
        skip_helper.rIndex_ld[i] = mask ? static_cast<uint32_t>(mIndex(make_coord(off, k_group_id))) : 0;
    }
    ...
    if constexpr (kWarpSkipMMA) {
        ...
        for (
            ...
            uint32_t rIndex_st1 = (off < M && mMask(make_coord(off, k_group_id)) > 0) ? static_cast<uint32_t>(mIndex(make_coord(off, k_group_id))) : M;
            uint32_t rIndex_st2 = (off + 8 < M && mMask(make_coord(off, k_group_id)) > 0) ? static_cast<uint32_t>(mIndex(make_coord(off + 8, k_group_id))) : M;

            skip_helper.execute_mma_warp[i] = static_cast<uint8_t>(__any_sync(0xffffffff, static_cast<int>((rIndex_st1 < M) || (rIndex_st2 < M))));
        }
    }
}
...
>
__global__ void gemm_k_v1_kernel(const __grid_constant__ GEMMParams params) {
    using namespace cute;
    ...
    using SharedStorageAB = typename MmaTraits_::SharedStorageAB;
    using SharedStorageD = typename MmaTraits_::SharedStorageD;
    ...
    // Process each K-group independently. This gives up cross-group prefetching
    // but lets fully inactive sorted groups skip the whole inner pipeline.
    CUTE_NO_UNROLL
    for (uint32_t k_group=0; k_group < KOutIter; ++k_group) {
        const uint32_t k_group_id = k_base_group + k_group;
        const uint32_t group_tile_base = k_base_tile + k_group * NGIter;
        ...
        update_skip_metadata<G2SAIter, kWarpSkipG2S, kWarpSkipMMA, G2SRowPerCTA, MMARow, MMACol, MMAIter, PipelineSmem, decltype(mMask), decltype(mIndex)>(
            skip_helper, mMask, mIndex, M, max_off_m, thread_off_m, base_off_m, k_group_id, warp_id, lane_id
        );
        ...
        for (uint32_t p=0; p < PipelineSmem - 1; ++p) {
            if (producer < NGIter) {
                ...
                for (uint32_t i=0, row_off=thread_off_m; i < size<1>(pDsA); ++i, row_off+=G2SRowPerCTA) {
                    if constexpr (kThrSkipG2S) {
                        if (skip_helper.rMask[i]) {
                            auto gAtA = gA(_, _, skip_helper.rIndex_ld[i], group_tile_base + producer);
                ...
                copy(g2sB_tiled_copy{}, pSgB(_, _, _, group_tile_base + producer), pDsB(_, _, _, make_coord(0, producer_stage)));
                cp_async_fence();
                ++prologue_cp_groups;
                ++producer;
        ...
        CUTE_UNROLL
        for (uint32_t k_inner=0; k_inner < NGIter; ++k_inner) {
            CUTE_UNROLL
        ...
        // Epilogue, split-k atomic-add write back for this active K-group.
...
struct GEMM_K_V1_Host {
    static void run(
        ...
        constexpr auto kernel = gemm_k_v1_kernel<
            kBM, kBN, kBK, 16, kPipeline, 2, kN, kK, kNG, kNGIter,
