...
from .triton import gemm_k_host as triton_ref_program

__SUPPORT_VERSION__ = list(cute_versions("gemm_k"))
__SUPPORT_ARCH__ = list(cute_architectures("gemm_k"))
...
def prune_gemm_k_space(configs: List[JitConfig], named_args: Mapping[str, Any]) -> list[JitConfig]:
    ...
    bm_candidate, bn_candidate = get_mn_candidate(rM, max_mn), get_mn_candidate(N, max_mn)
    ...
    for config in configs:
        ...
        if (BM > 16 and BN > 16) and BK > 32: continue
        if (BM <= 16 or BN <= 16) and (BK <= 32 and kwargs['ThrSkipG2S'] == False): continue # must use (16,64) for (16, 8) copy to smem [16, 64], or skip OOB loading
        if G % BK != 0 or SplitK > (K // G): continue
        ...
        if K % SplitK != 0 or (K // SplitK) % G != 0: continue
        filtered.append(config)

    return filtered

### Heuristic Fallback
def heuristic_gemm_k_config(named_args: Mapping[str, Any]) -> dict[str, Any]:
    cc = int(named_args["cc"])
    smem_size = int(named_args["smem_size"])
    num_sm = int(named_args["num_sm"])
    data_width = int(named_args["data_width"])
    ...
    while data_width * Pipeline * (BM * BK + BN * BK) > smem_size / 2:
        if BN > 128:
            BN >>= 1
        elif (BM > 16 and BK <= 32):
            BM >>= 1
        elif (BN > 16 and BK <= 32):
            BN >>= 1
        else:
            break
...
def _compile_gemm_k_module(
    ...
    cpp_args = [N, K, NG, G // BK, BM, BN, BK, Pipeline, ThrSkipG2S, WarpSkipG2S, WarpSkipMMA] + [is_arch_support_pdl(), dtype]
    ...
    return load_jit_for_arch(
        ...
        "gemm_k",
        *cpp_args,
        cuda_files=[str(ROOT_PATH / "include" / f"gemm_k/v{kernel_version}/{arch}.cuh")],
        cuda_wrappers=[("kernel", f"GEMM_K_V{kernel_version}_Host<{cpp_args}>::run")],
...
@autotune(
    ...
    configs=gemm_k_space,
    key=['rM', 'N', 'K', 'NG', 'G', 'dtype', 'kernel_version', 'arch', 'estimate_sparsity', 'cc'],
    prune_configs_by=prune_gemm_k_space,
    runtime_params=["A_f", "B", "Mask_s", "Index", "D", "estimate_sparsity"],
    restore_params=["D"],
    heuristic=heuristic_gemm_k_config,
    cudagraph=True,
)
def _jit_gemm_k_module(
    N: int, K: int, NG: int, G: int, BM: int, BN: int, BK: int, Pipeline: int,
    ThrSkipG2S: bool, WarpSkipG2S: bool, WarpSkipMMA: bool,
    kernel_version: int,
    arch: str,
...
### Search Space Pruning
def prune_gemv_k_space(configs: List[JitConfig], named_args: Mapping[str, Any]) -> list[JitConfig]:
    return configs

### Heuristic Fallback
def heuristic_gemv_k_config(named_args: Mapping[str, Any]) -> dict[str, Any]:
    cc = int(named_args["cc"])
...
def _compile_gemv_mn_module(
    N: int, K: int, NG: int, G: int, BN: int, Pipeline: int,
    kernel_version: int,
    arch: str,
    dtype: Optional[torch.dtype]=torch.float16,
) -> Module:
    ...
    cpp_args = [N, K, NG, G, BN, Pipeline] + [is_arch_support_pdl(), dtype]
    cpp_args = make_cpp_args(*cpp_args)
    ...
    return load_jit_for_arch(
        ...
        "gemv_k",
        *cpp_args,
        cuda_files=[str(ROOT_PATH / "include" / f"gemv_k/v{kernel_version}/{arch}.cuh")],
        cuda_wrappers=[("kernel", f"GEMV_K_V{kernel_version}_Host<{cpp_args}>::run")],
        extra_cflags=DEFAULT_CFLAGS,
        extra_cuda_cflags=DEFAULT_CUDA_CFLAGS,
        extra_include_paths=THIRD_PARTY_HEADER_DIRS,
    )
...
@autotune(
    kernel_id="gemv_k",
    config_params=['BN', 'Pipeline'],
    configs=gemv_k_space,
    key=['M', 'N', 'K', 'NG', 'G', 'dtype', 'kernel_version', 'arch', 'activation', 'estimate_sparsity', 'cc'],
    prune_configs_by=prune_gemv_k_space,
    runtime_params=["A_f", "B", "Mask_f", "D", "estimate_sparsity"],
    restore_params=["D"],
    heuristic=heuristic_gemv_k_config,
    cudagraph=True,
)
def _jit_gemv_k_module(
    N: int, K: int, NG: int, G: int, BN: int, Pipeline: int,
    kernel_version: int,
    arch: str,
    dtype: Optional[torch.dtype]=torch.float16,
    ...
    return _compile_gemv_mn_module(
        N, K, NG, G, BN, Pipeline,
        kernel_version, arch,
        dtype=dtype,
    )


def _use_gemv_k_dispatch(M: int, G: int) -> bool:
    if M == 1:
        return G >= 256
    if M == 2:
        return G >= 128
...
def gemm_k(
    ...
    metadata = {}
    if _use_gemv_k_dispatch(M, G):
        Mask_f = Mask.flatten(0, 1)
        module = _jit_gemv_k_module(
            M=M,
            N=N,
            K=K,
            NG=NG,
...
def torch_ref_program(
    ...
    **kwargs,
    ...
    if Mask is not None:
        ...
        AG = rearrange(A, 'b t (ng d) -> b t ng d', ng=Mask.shape[-1])
        AG.masked_fill_(Mask.logical_not()[:, :, :, None], 0)
        AG = rearrange(AG, 'b t ng d -> (b t) (ng d)')
        D = AG @ B.T
    else:
        D = A.flatten(0, 1) @ B.T
    
    return rearrange(D, '(b t) d -> b t d', b=A.shape[0])
...
def run_once(**kwargs):
    ...
    Mask = torch.rand((1, M, K // G), device=device) >= sparsity
    ...
    D_triton = triton_ref_program(
        deepcopy(A), deepcopy(B), deepcopy(Mask), None, estimate_sparsity=sparsity
    )
    ...
    for version in kwargs.get('kernel_version'):
        ...
        D_cute, _ = gemm_k(
            deepcopy(A), deepcopy(B), deepcopy(Mask), None, estimate_sparsity=sparsity, **compile_kwargs
        )
...
def _benchmark_interface(**kwargs):
    ...
    B = torch.empty((N, K), dtype=dtype, device=device)
    torch_init.kaiming_uniform_(B, a=math.sqrt(5))
    Mask = torch.rand((1, M[-1], K // G), device='cpu')
