...
from .triton import attention_head_host as triton_ref_program

__SUPPORT_VERSION__ = list(cute_versions("attention_prefill"))
__SUPPORT_ARCH__ = list(cute_architectures("attention_prefill"))
...
def prune_attention_head_space(configs: List[JitConfig], named_args: Mapping[str, Any]) -> list[JitConfig]:
    ...
    max_mn = 128
    bm_candidate, bn_candidate = get_mn_candidate(rTq, max_mn), get_mn_candidate(rTk, max_mn)
    ...
    for config in configs:
        ...
        if BM not in bm_candidate or BN not in bn_candidate: continue
        if (BM <= 16 or BN <= 16) and (D <= 32 and kwargs['ThrSkipG2S'] == False): continue # must use (16,64) for (16, 8) copy to smem [16, 64], or skip OOB loading
        if data_width * (PipelineK * (BN * D) + PipelineV * (BN * D)) > smem_size: continue
        if cc // 10 in [8, 12] and data_width * (PipelineK * (BN * D) + PipelineV * (BN * D)) > smem_size / 2: continue
        if (BM // (int(kwargs["Warps"]) * 16)) != 1 and (BM <= 64): continue
        if (BM > 64) and int(kwargs["Warps"]) < 4: continue
...
def heuristic_attention_head_config(named_args: Mapping[str, Any]) -> dict[str, Any]:
    ...
    BM = min(128, max(16, rTq))
    BN = min(64, max(16, rTk))
    PipelineK = 2
    PipelineV = 1
    Warps = min(4, BM // 16)
    ...
    while data_width * (PipelineK * (BN * D) + PipelineV * (BN * D)) > smem_size / 2:
        if PipelineK > 1:
            PipelineK -= 1
        elif BN >= 64:
            BN >>= 1
    ...
    return {
        ...
        "PipelineV": PipelineV,
        "Warps": Warps,
        "ThrSkipG2S": False if rTq > 16 else True,
        "WarpSkipG2S": False,
        "WarpSkipMMA": False,
    }
...
def _compile_attention_head_module(
    ...
    BM: int, BN: int, PipelineK: int, PipelineV: int, Warps: int,
    IsCausal: bool, IsLeftpad: bool, 
    ...
    cpp_args = [Hq, Hk, D, NG, BM, BN, PipelineK, PipelineV, Warps, IsCausal, IsLeftpad, ThrSkipG2S, WarpSkipG2S, WarpSkipMMA] + [is_arch_support_pdl(), dtype]
    ...
    return load_jit_for_arch(
        ...
        "attention_head",
        *cpp_args,
        cuda_files=[str(ROOT_PATH / "include" / f"attention_prefill/v{kernel_version}/{arch}.cuh")],
...
@autotune(
    ...
    config_params=['BM', 'BN', 'PipelineK', 'PipelineV', 'Warps', 'ThrSkipG2S', 'WarpSkipG2S', 'WarpSkipMMA'],
    configs=attention_head_space,
    key=['B', 'rTq', 'rTk', 'Hq', 'Hk', 'D', 'NG', 'IsCausal', 'IsLeftpad', 'dtype', 'kernel_version', 'arch', 'estimate_sparsity', 'cc'],
    prune_configs_by=prune_attention_head_space,
    runtime_params=["Q", "K", "V", "Mask_s", "Index", "Leftpad", "O", "scale", "estimate_sparsity"],
    restore_params=["O"],
...
def _jit_attention_head_module(
    ...
    return _compile_attention_head_module(
        Hq, Hk, D, NG, BM, BN, PipelineK, PipelineV, Warps,
        IsCausal, IsLeftpad,
...
def attention_head(
    ...
    if sorted_indices is None:
        ...
        Mask_s = sorted_mask.view(B, Tq, NG)
        Index = sorted_indices.view(B, Tq, NG)
    
    module = _jit_attention_head_module(
        B=B, rTq=next_pow_of_2(Tq), rTk=next_pow_of_2(Tk), Hq=Hq, Hk=Hk, D=D, NG=NG,
        IsCausal=is_causal, IsLeftpad=is_leftpad,
    ...
    module.run(Q, K, V, Mask_s, Index, Leftpad, O, scale, estimate_sparsity)
    return O, dict(sorted_mask=Mask_s, sorted_indices=Index)
...
def run_once(**kwargs):
    ...
    B = kwargs.get("B", [1])[-1]
    Tq = kwargs.get("Tq", [8192])[-1]
    Tk = kwargs.get("Tk", [])
    if len(Tk) > 0 and Tk[0] == -1: Tk = []
    Tk = Tq if len(Tk) == 0 else Tk[-1]
    Hq = kwargs.get("Hq", 32)
    Hk = kwargs.get("Hk", 8)
    D = kwargs.get("D", 128)
    NG = kwargs.get("NG", 8)
    ...
    D_torch = torch_ref_program(
        deepcopy(Q), deepcopy(K), deepcopy(V), deepcopy(Mask),
        is_causal=is_causal,
        estimate_sparsity=sparsity,
    )
...
def _benchmark_interface(**kwargs):
    ...
    B = kwargs.get("B", [1])
    Tq = kwargs.get("Tq", [8192])
    Tk = kwargs.get("Tk", [])
    if len(Tk) > 0 and Tk[0] == -1: Tk = []
    tk_follows_tq = len(Tk) == 0
    if tk_follows_tq: Tk = Tq
    Hq = kwargs.get("Hq", 32)
    Hk = kwargs.get("Hk", 8)
    D = kwargs.get("D", 128)
    NG = kwargs.get("NG", 8)
    ...
    assert not (len(B) > 1 and len(Tq) > 1 and len(sparsity) > 1), "Each profiling step only support one variable"
    if len(Tq) > 1:
        assert len(Tk) == 1 or Tq == Tk, "Each profiling step only support one variable"

    variable = dict(B=B)
    if len(Tq) > 1: variable = dict(Tq=Tq)
    if len(Tk) > 1 and Tk != Tq: variable = dict(Tk=Tk)
    elif len(sparsity) > 1: variable = dict(sparsity=sparsity)

    Q = torch.randn((B[-1], Tq[-1], Hq, D), dtype=dtype, device=device)
    K = torch.randn((B[-1], Tk[-1], Hk, D), dtype=dtype, device=device)
    V = torch.randn((B[-1], Tk[-1], Hk, D), dtype=dtype, device=device)
    Mask = torch.rand((B[-1], Tq[-1], NG), device='cpu')

    benchmark_kwargs = dict(
        B=B[-1], Tq=Tq[-1], Tk=Tk[-1], Hq=Hq, Hk=Hk, D=D, NG=NG,
        Q=Q, K=K, V=V, Mask=Mask, sparsity=sparsity[-1], is_causal=is_causal,
        tk_follows_tq=tk_follows_tq,
        check_precision=kwargs.get("check_precision", True),
        res_cache=dict(),
        compile_kwargs=dict(
            arch=kwargs.get("arch"),
    ...
    def benchmark(**bench_kwargs):
        ...
        current_tk = current_tq if bench_kwargs.get("tk_follows_tq") else bench_kwargs.get("Tk")
        ...
        if bench_kwargs.get("is_causal"):
            tflops = lambda ms: (2 * (run_args["B"] * run_args["Tq"] * run_args["Tk"] * run_args["Hq"] * run_args["D"]) * 1e-12) / (ms * 1e-3)
        else:
            tflops = lambda ms: (2 * 2 * (run_args["B"] * run_args["Tq"] * run_args["Tk"] * run_args["Hq"] * run_args["D"]) * 1e-12) / (ms * 1e-3)
